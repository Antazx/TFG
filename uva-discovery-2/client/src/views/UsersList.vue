    ...
    <v-card>
      <v-card-title>
        <v-row>
          ...
          <v-spacer></v-spacer>
          <v-col>
            <UserComponent :currentUser="{}"></UserComponent>
          </v-col>
          <v-col>
            <v-btn class="mx-2" dark medium color="primary" @click="getUsers">
      ...
      <v-data-table
        ...
        class="elevation-1"
      >
        <template v-slot:item.lastlogin="{ item }">{{ item.lastlogin | moment("from", "now") }}</template>
