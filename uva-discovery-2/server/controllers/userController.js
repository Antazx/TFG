...
const createUser = async (userData) => {
  ...
  const hashedPassword = await hashPassword(password);
  const newUser = new User({ name: email, email, password: hashedPassword, role: 'basic' });
  const accessToken = jwt.sign({ _id: newUser._id, role: newUser.role }, process.env.JWT_SECRET, {
    expiresIn: '1d',
  });
  newUser.accesstoken = accessToken;
...
const login = async (req, res, next) => {
  try {
    ...
    ldapConnection(email, password)
      ...
      .catch((error) => {
        logger.error(`LDAP login failed: ${error}`, FROM);
        logger.info('Checking email, password against BD', FROM);
        ldapLoginOk = false;
      });

    let user = await User.findOne({ email });
    if (!user && !ldapLoginOk) return next({ message: 'There is no user with that email', status: 401 });
    if (!user && ldapLoginOk) user = await createUser(email, password);
  ...
  return async (req, res, next) => {
    try {
      ...
      const permission = await ac
        .can(role)
        .context({ requester: requester, reservedby: reservedby })
        .execute(action)
        .on(resource);

      logger.info(`requester: ${user.email} reservedby: ${reservation.reservedby}`, FROM);
